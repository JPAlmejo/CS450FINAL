...
//global boolean that changes if the texture is distorted or not
bool Distort = false;
struct point
{
...
OsuSphere( float radius, int slices, int stacks )
{
	// set the globals:
	float timer;
	int milisec = glutGet(GLUT_ELAPSED_TIME);
	milisec %= 10000;
	timer = (float)milisec / (float)10000;		// [0.,1.)
	...
	for( int ilat = 0; ilat < SphNumLats; ilat++ )
	{
											...
											// ilng=SphNumLngs-1, lng=+M_PI are the same meridian
		{
			...
			else
			{
					srand(time(0));
					p->s = (lng + M_PI) / (2. * M_PI);		// the correct code
					p->t = ((lat + M_PI / 2.) / M_PI) * (sin(timer) / p->s);		// the correct code
			}
			//====================================================
			//=====================================================
		}
